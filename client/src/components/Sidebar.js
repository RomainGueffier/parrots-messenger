...
export default function Sidebar({ id, setToggleSidebar, sidebarVisibility }) {
  ...
  return (
    <div
      className={`${sidebarVisibility} d-flex flex-column bg-light text-success p-3`}
    >
      ...
      <div className="d-flex flex-column h-100">
        <small className="text-muted d-block my-2">
          Ton identifiant est : <u>{id}</u>. Partage le avec la personne avec
          qui tu veux discuter.
        </small>
        <Button onClick={() => window.localStorage.clear()} variant="warning">
